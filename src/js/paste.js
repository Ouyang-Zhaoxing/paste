var jsonbinKey = "$2b$10$kJPWricyWsFwxrp8PxwvdOS7yXESfKCJw3q1U1KqUkxzM/XUr6AOm";
...
function viewCode(codeID) {
    ...
    req.open("GET", "https://api.jsonbin.io/b/" + codeID, true);
    req.setRequestHeader("secret-key", jsonbinKey);
    req.send();
...
function pasteCode() {
    ...
    req.onreadystatechange = function () {
        if (req.readyState === XMLHttpRequest.DONE && req.status === 200) {
            var codeID = JSON.parse(req.response).id.toString();
            var resUrl = host + "?id=" + codeID;
            setTimeout(function () { location.href = resUrl; }, 300);
        }
    };
    req.open("POST", "https://api.jsonbin.io/b", true);
    req.setRequestHeader("Content-Type", "application/json");
    req.setRequestHeader("secret-key", jsonbinKey);
    req.setRequestHeader("collection-id", "5fe90b45c8f3567b4a1f2d73");
